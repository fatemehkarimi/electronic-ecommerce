...
function CustomCard({ product }) {
  return (
    <Card image={ product[PConst.PRODUCT_STANDARD_IMAGE] }>
      <span>
        { formatLongString(product[PConst.PRODUCT_NAME]) }
      </span>
      <Review 
...
function CategoryProductList() {
  ...
  const { products, totalPages, loading, error } = 
    useCategoryProductsFetch(categoryId, 1);
    ...
    <div className='category-product-list'>
      ...
      <div className='category-product-list-content'>
        <div className="category-product-grid-container">
          ...
          products.map((item) => {
            return (
              <a className='simple-link' href={ `/product/${item[PConst.PRODUCT_IDENTITY_KEY]}` } >
